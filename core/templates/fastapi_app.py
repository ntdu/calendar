...
ws_connector_page = """
    ...
    <script src="https://cdn.socket.io/4.5.0/socket.io.min.js" integrity="sha384-7EyYLQZgWBi67fBtVxw60/OWl1kjsfrPFcaU0pp0nAh+i8FD068QogUvg85Ewy1k" crossorigin="anonymous"></script>    # noqa
    <script>
...
def create_fastapi_app(     # NOSONAR
    enable_api: bool = True,    # NOSONAR
    ...
    **kwargs
    ...
    service_home_page = default_page.format(
        ...
        zalo_domain_verification_code=kwargs.get('zalo_domain_verification_code')
    )
    ...
    async def app_service_set_log_level(name: str, level: int = logging.INFO):
        ...
        if name not in (
            constants.DEBUG_LOGGER,
            constants.CONSOLE_LOGGER
        ):
            return {'success': False}
    ...
    if enable_api:
        ...
        api_app = create_api_app(api_router, middlewares, docs_url, redoc_url, openapi_url)
        app.mount(api_mount_path, api_app)

    # Websocket App
    if enable_ws:
        if not callable(ws_event_handlers_initializer):
            raise ValueError('expected ws_event_handlers_initializer is a callable object')
